...
void getTagPose(const matd_t& H,
             const Mat3& Pinv,
...
AprilTabPoseEstimationNode::AprilTabPoseEstimationNode(const rclcpp::NodeOptions& options)
  : Node("apriltag", options),
    ...
    // topics
    sub_cam(image_transport::create_camera_subscription(this, "image_rect", std::bind(&AprilTabPoseEstimationNode::onCamera, this, std::placeholders::_1, std::placeholders::_2), declare_parameter("image_transport", "raw", descr({}, true)), rmw_qos_profile_sensor_data)),
...
void AprilTabPoseEstimationNode::onCamera(const sensor_msgs::msg::Image::ConstSharedPtr& msg_img,
                            const sensor_msgs::msg::CameraInfo::ConstSharedPtr& msg_ci)
{
    ...
    for(int i = 0; i < zarray_size(detections); i++) {
        ...
        getTagPose(*(det->H), Pinv, tf.transform, tag_sizes.count(det->id) ? tag_sizes.at(det->id) : tag_edge_size);
